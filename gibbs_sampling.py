...
# -------------------------
# 1. 对单个测试点局部超参数采样（更新 φₜ = [w, logtheta]）
# -------------------------
# def potential_local_single(phi, jump_result):
#     """
#     计算单个测试点局部目标函数：phi = [w, logtheta] 的负 log 后验
#     jump_result 包含 "zeta_t", "f_t", "jumpGP_model"（其中包含 "r", "ms"）
#     """
#     Q = jump_result["zeta_t"].shape[1]
#     M = jump_result["zeta_t"].shape[0]
#     w = phi[:1+Q]
...
#     model = jump_result["jumpGP_model"]
#     r = model["r"].flatten().to(torch.float64)  # (M,)
#     f_t = jump_result["f_t"].to(torch.float64)    # (M,)
#     zeta = jump_result["zeta_t"].to(torch.float64)  # (M, Q)
#     ms = model["ms"]
...
#     # f_t likelihood：利用多元正态分布
#     K = compute_jumpGP_kernel_torch(zeta, logtheta)
#     mean_vec = ms * torch.ones(M, device=zeta.device, dtype=zeta.dtype)
#     def log_multivariate_normal_pdf_torch(x, mean, cov):
#         cov = cov.to(torch.float64)
#         diff = x - mean
#         sign, logdet = torch.slogdet(cov)
...
#             sign, logdet = torch.slogdet(cov)
#         return -0.5 * (diff @ torch.inverse(cov) @ diff + logdet + len(x)*math.log(2*math.pi))
#     loglik_f = log_multivariate_normal_pdf_torch(f_t, mean_vec, K)
    
#     # 先验：假设 p(w) ~ N(0,I), p(logtheta) ~ N(0,I)
#     log_prior = -0.5 * torch.sum(w**2) - 0.5 * torch.sum(logtheta**2)
    
#     log_post = loglik_r + loglik_f + log_prior
#     return -log_post  # 返回负的 log 后验
...
# -------------------------
# 3. 并行更新所有测试点的局部参数（使用 ProcessPoolExecutor）
# -------------------------
# def parallel_update_all_test_points(jump_gp_results, U=0.1, local_step_size=0.01, local_num_steps=10, local_num_samples=1, local_warmup_steps=50, num_workers=4):
#     def update_one(j):
#         # 注意：使用 ProcessPoolExecutor 时，需要将 jump_gp_results[j] 转换为可pickle对象
#         return update_one_test_point(jump_gp_results[j], U, local_step_size, local_num_steps, local_num_samples, local_warmup_steps)
    
#     with ProcessPoolExecutor(max_workers=num_workers) as executor:
#         results = list(executor.map(update_one, range(len(jump_gp_results))))
#     return results
